...
def add_produced_timestamp(message: dict[str, Any]) -> dict[str, Any]:
    """Add a produced-at timestamp to the message."""
    message["produced_at"] = datetime.utcnow().isoformat() + "Z"
    return message
...
def send_messages() -> int:
    ...
    try:
        for message in generate_messages(MESSAGE_COUNT):
            message = add_produced_timestamp(message)
